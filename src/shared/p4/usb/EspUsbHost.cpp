...
#if BOARD_INPUT_USB && BOARD_PANEL_DSI   // P4 only (see EspUsbHost.h); empty on other boards
    ...
    case USB_INTERFACE_DESC:
      {
        ...
        // P4 patch: only claim HID interfaces (class 0x03). Composite devices (e.g. a multimedia
        // keyboard+touchpad) also expose AUDIO interfaces with isochronous endpoints; claiming those
        // and leaving their ISOC bandwidth unserviced destabilizes the host and can stop the mouse
        // interrupt transfers from completing. We only need keyboard/mouse/gamepad anyway.
        if (intf->bInterfaceClass != 0x03) {       // 0x03 = HID
          this->claim_err = ESP_ERR_NOT_SUPPORTED;  // skip; its endpoints are skipped via claim_err below
          break;
        }
